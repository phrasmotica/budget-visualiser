...
func load_latest_budget() -> BudgetData:
	...
	if budget_files.size() <= 0:
		CustomLogger.info("No saved data to load!")
		return null
...
func save_budget_data(data: BudgetData) -> void:
	...
	if success:
		CustomLogger.info("Saved data successfully!")
...
func _save_data(data: BudgetData) -> bool:
	...
	CustomLogger.info("Saved budget data to %s" % file_name)
...
func _load_data(file_name: String) -> BudgetData:
	...
	if not FileAccess.file_exists(path):
		CustomLogger.info("Budget file %s does not exist!" % file_name)
		return null

	var data: BudgetData = ResourceLoader.load(path)

	CustomLogger.info("Loaded budget data from %s" % file_name)
